...
function ProjectsCard({ project, index }) {
    ...
    return (
        <article className={`projects-card__article displayed${index}`} index={index} key={project.id} >

            <div className={isImageClicked ? 'lightbox open' : 'lightbox'} onClick={closeLightbox}>
                <img className='lightbox-content' src={project.fullImg || selectedProjectImage} alt="Enlarged project" />
